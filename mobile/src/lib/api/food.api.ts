...
import type { IScanFoodResponse, IFoodLogItem, IFoodLog, IFoodItem, IFoodDaySummary } from './types';
...
export async function uploadFoodLogImageApi(logId: string, imageUri: string): Promise<string | null> {
  const formData = new FormData();
  formData.append('image', { uri: imageUri, type: 'image/jpeg', name: 'meal.jpg' } as unknown as Blob);
  const res = await apiClient.patch(`/api/food/logs/${logId}/image`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
    timeout: 30000,
  });
...
export async function getFoodLogsRangeApi(from: string, to: string): Promise<IFoodDaySummary[]> {
  const res = await apiClient.get('/api/food/logs/range', { params: { from, to } });
  return res.data.data as IFoodDaySummary[];
}
